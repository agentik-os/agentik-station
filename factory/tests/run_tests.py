...
chk('builder profile', lambda: (_ for _ in ()).throw(Exception('missing')) if not (ROOT.parent/'os/builder/BUILDER_PROFILE.md').exists() else None)
chk('librarian profile', lambda: (_ for _ in ()).throw(Exception('missing')) if not (ROOT.parent/'os/librarian/director/PROFILE.md').exists() else None)
chk('schemas parse', lambda: [json.loads(p.read_text()) for p in (ROOT/'schemas').glob('*.json')])
