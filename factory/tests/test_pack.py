...
def test_core_files_exist():
    ...
    assert (ROOT.parent/'packages/os/builder-os/README.md').exists()
    assert (ROOT.parent/'packages/os/librarian-os/README.md').exists()

def test_example_schemas_json_parse():
    for p in (ROOT/'06_SCHEMAS').glob('*.json'): json.loads(p.read_text())
