...
def test_core_role_compiles_expected_canonical_zones() -> None:
    ids = _zone_ids(InstallSpec(host_id="gareth-core-01", role="core"))
    assert {"station-maintainer", "discord-bootstrap", "fleet-operator", "gareth", "dev", "os", "hermes-edge"} <= ids
...
def test_client_host_compiles_only_system_and_requested_client_zone() -> None:
    ...
    ids = _zone_ids(spec)
    assert ids == {"station-maintainer", "discord-bootstrap", "fleet-operator", "moonbase-prod"}
    assert "gareth" not in ids and "dev" not in ids and "os" not in ids


def test_plan_is_typed_and_ends_at_ready_for_setup() -> None:
    spec = InstallSpec(host_id="gareth-core-01", role="core")
