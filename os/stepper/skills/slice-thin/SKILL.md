...
Validate the supplied input with python3 -I -B ABSOLUTE_RUNNER validate --skill slice-thin --kind input --input ABSOLUTE_WORKSPACE_INPUT.json. Perform the declared reasoning without changing accounts, services, external trackers or source software. Emit the complete typed artifact, including mandatory confidence A–E, evidence references and assumptions. Validate the input/output transition with python3 -I -B ABSOLUTE_RUNNER transition --skill slice-thin --input ABSOLUTE_WORKSPACE_INPUT.json --output ABSOLUTE_WORKSPACE_OUTPUT.json before returning it to the Director. Empty evidence means no source verification, never accepted confidence by itself. Preserve the exact supplied outcome. Reject duplicate source activity/task identities and cover the supplied journey; do not silently drop its final confirmation step.
