---
name: story-map
description: Map activities and tasks left to right.
---

# Skill: story-map

**Purpose:** Map activities and tasks left to right.

**When to use:** The backlog lost the plot.

**Steps:**
1. Name the user and the journey.
2. Lay activities left to right.
3. Fill tasks under each.
4. Mark already-true vs missing.

**Failure modes:**
- Feature inventory posing as a map
...
Validate the supplied input with python3 -I -B ABSOLUTE_RUNNER validate --skill story-map --kind input --input ABSOLUTE_WORKSPACE_INPUT.json. Perform the declared reasoning without changing accounts, services, external trackers or source software. Emit the complete typed artifact, including mandatory confidence A–E, evidence references and assumptions. Validate the input/output transition with python3 -I -B ABSOLUTE_RUNNER transition --skill story-map --input ABSOLUTE_WORKSPACE_INPUT.json --output ABSOLUTE_WORKSPACE_OUTPUT.json before returning it to the Director. Empty evidence means no source verification, never accepted confidence by itself. Preserve the supplied role as the map actor and preserve the original journey; clarify changed scope before producing a different map.
