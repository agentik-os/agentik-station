...
Validate the supplied input with python3 -I -B ABSOLUTE_RUNNER validate --skill shape-bet --kind input --input ABSOLUTE_WORKSPACE_INPUT.json. Perform the declared reasoning without changing accounts, services, external trackers or source software. Emit the complete typed artifact, including mandatory confidence A–E, evidence references and assumptions. Validate the input/output transition with python3 -I -B ABSOLUTE_RUNNER transition --skill shape-bet --input ABSOLUTE_WORKSPACE_INPUT.json --output ABSOLUTE_WORKSPACE_OUTPUT.json before returning it to the Director. Empty evidence means no source verification, never accepted confidence by itself. Preserve the exact supplied problem and appetite amount/unit; an estimate or rewritten output must not replace the problem or enlarge that limit.
