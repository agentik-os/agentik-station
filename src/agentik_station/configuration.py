...
def _validate_supported_platform(value: Any) -> None:
    ...
    if value.get("requires_systemd") is not True:
        raise ValidationError("v11 supported_platform.requires_systemd must be true")
    if value.get("package_manager") != "apt":
        raise ValidationError("v11 supported_platform.package_manager must be apt")
...
def _load_os_ids(repo_root: Path) -> set[str]:
    payload = load_os_catalog(repo_root / "packages" / "os" / "CATALOG.json")
