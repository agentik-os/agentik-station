...
jq -n --arg user "$STATION_USER" --arg hermes "$hermes_version" --arg codex "$codex_version" --arg repo "$REPO_DIR" --arg mode "$MODE" '{station_user:$user,hermes:$hermes,codex:$codex,repository:$repo,mode:$mode}' > /etc/station/bootstrap-tools.json
