...
+ orchestration contract + capability lanes
...
+ connector/capability readiness probes
+ deliverable render/quality gates when applicable
...
+ Librarian domain research + Builder handoff + provenance
...
Every OS declares which Station capability lanes it activates and which evidence is required to claim completion. The common lanes are Clarify/Plan, Build with Leverage, Research/Learn, Code/Ship Safely, Polished Deliverables, Remember/Operate and Connect with Clear Boundaries.

The OS must preserve claim semantics:

```text
prepared -> observed -> reported -> verified -> read_back -> accepted
```

A mission may skip non-applicable stages, but it may never relabel weaker evidence as stronger evidence. Connector readiness and deliverable render gates are part of this contract when the OS depends on external systems or creates user-facing artifacts.
