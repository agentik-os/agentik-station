...
The resolved result includes `current_version`, `installed_version`,
`source_version_matches`, `source_roles_match`, the immutable compiled distribution
and its trusted bundle hash. Matching versions/roles allow current-package routing;
they are not a claim of recompilation-byte equality or live provider acceptance.
Instance chat and instance/legacy gateway install/start/restart reject mismatched
versions or teams. Configuration and observation remain accessible for repair.
...
AGK now treats Builder/BuilderOS/master-os-builder, Stepper and Librarian requests
as canonical Station OS requests. Its generic operator-local agent tool and OS
view do not possess a trusted cross-UID instance launcher: they return an explicit
scoped Station resolve/chat plan without executing it. They never substitute a
generic legacy Hermes session or guess a client Zone. The old same-operator
Builder is available only through the explicit `legacy-master-os-builder`
compatibility alias; its existing session namespace is preserved. The generic
Agents view remains an operator catalog, not the canonical instance registry.

## Complete Builder Suite publication

[`agentik-os/agk-builder-os`](https://github.com/agentik-os/agk-builder-os) distributes
the complete Builder, Librarian and Stepper source trees together under
`station-suite/os/`, with an immutable Station commit and exact file hashes.
Canonical edits remain here under `os/`; the external suite is generated by
`scripts/builder_suite.py`. All three are mandatory in every exported snapshot.
They retain separate native Directors and instance namespaces, not shared accounts.

The source snapshot is not a native Hermes distribution or standalone compiler.
Install through the pinned Station release, which supplies shared skills, rules,
web plugins, resource catalogs and compiler dependencies. The standalone verifier
supports offline checks and optional exact-commit/current-package comparison with
an explicit local Station checkout. It never fetches, authenticates, installs or
overwrites a changed destination. Existing older external packages remain history,
not the current Station VPS deployment path.
