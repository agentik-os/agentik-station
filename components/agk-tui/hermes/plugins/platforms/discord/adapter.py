...
def _define_discord_view_classes() -> None:
    ...
    class ModelPickerView(discord.ui.View):
        ...
        async def _on_provider_selected(self, interaction: discord.Interaction):
            ...
            pages = max(1, (len(provider.get("models", [])) + 24) // 25) if provider else 1
            page_text = f" (page 1/{pages})" if pages > 1 else ""

            await interaction.response.edit_message(
                embed=discord.Embed(
                    title="⚙ Model Configuration",
                    description=f"Provider: **{pname}**\nSelect a model{page_text}:",
                    color=discord.Color.blue(),
                ),
                view=self,
            )
        ...
        async def _change_model_page(
            ...
            self._build_model_select(self._selected_provider, page=target_page)
            ...
            await interaction.response.edit_message(
                embed=discord.Embed(
                    ...
                    description=(
                        f"Provider: **{pname}**\n"
                        f"Select a model (page {self._model_page + 1}/{pages}):"
                    ),
                    color=discord.Color.blue(),
                ),
                view=self,
        ...
        async def _on_model_page_prev(self, interaction: discord.Interaction):
            await self._change_model_page(interaction, -1)

        async def _on_model_page_next(self, interaction: discord.Interaction):
            await self._change_model_page(interaction, 1)
