#!/usr/bin/env python3
"""Explicit two-file operator refresh; no credentials, registries or services."""
...
# Exact Station 11.22 controls. Customized installed files require human review.
PREVIOUS = {
    "bin/agk": "f86d05b8e2c014056eb49e362bffcac2c9c73755536ebd5699b4b59364b68df8",
    "scripts/agk_control.py": "5ae627aa79d2eca21194b0b735dbee5030039c3a498526ec3f3b262f5773133d",
}
MAX_BYTES = 256 * 1024
...
def _read(parent: int, name: str, owners: set[int]) -> bytes:
    fd = os.open(name, os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK, dir_fd=parent)
    try:
        info = os.fstat(fd)
        if (not stat.S_ISREG(info.st_mode) or info.st_nlink != 1 or info.st_uid not in owners
                or info.st_mode & 0o022 or not info.st_mode & 0o100 or info.st_size > MAX_BYTES):
...
def refresh_controls(source: Path, prefix: Path) -> dict:
    ...
    prepared = []
    for relative, previous in PREVIOUS.items():
        source_file = source / relative
        target = prefix / ("bin/agk" if relative == "bin/agk" else "lib/agk-terminal/scripts/agk_control.py")
        with _parent(source_file.parent, uid=account.pw_uid, anchor=source) as parent:
            content = _read(parent, source_file.name, {0, account.pw_uid})
        with _parent(target.parent, uid=account.pw_uid, anchor=home) as parent:
            old = _read(parent, target.name, {account.pw_uid})
        if old != content and hashlib.sha256(old).hexdigest() != previous:
            raise ValueError("Installed controls were customized or are not the reviewed 11.22 version; preserved for review")
        prepared.append((target, old, content))
    changed = []
    # Both files are checked before the first mutation. This is not a global
    # transaction: a filesystem failure after one replacement requires retry.
    for target, old, content in prepared:
        with _parent(target.parent, uid=account.pw_uid, anchor=home) as parent:
            if _read(parent, target.name, {account.pw_uid}) != old:
            ...
            try:
                ...
                try:
                    ...
                    pass
            if _read(parent, target.name, {account.pw_uid}) != content:
